...
public enum Piece {
  ...
  private Piece(int id, int[][] movement, boolean tobi, int promotedPieceID, int demotedPieceID) {
    this.id = id;
  ...
  public static Piece valueOf(final int value) {
    for (Piece p : EnumSet.allOf(Piece.class)) {
      if (p.id == value) {
        return p;
      }
    }
